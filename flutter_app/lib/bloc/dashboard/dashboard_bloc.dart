...
class DashboardBloc extends Bloc<DashboardEvent, DashboardState> {
  ...
  DashboardBloc({
    ...
    on<CheckHandleAvailabilityEvent>((event, emit) async {
      try {
        final isAvailable = await blockbookService.isHandleAvailable(event.handle);
        if (isAvailable) {
          emit(HandleAvailable(event.handle));
        } else {
          emit(HandleTaken(event.handle));
        }
      } catch (e) {
        emit(DashboardError('Handle availability check failed: $e'));
      }
    });
